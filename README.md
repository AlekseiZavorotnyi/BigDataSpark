...
В проекте реализована обязательная часть: PostgreSQL, Spark и ClickHouse.
...
2. Запустить Spark ETL из сырой таблицы `mock_data` в модель звезда в PostgreSQL:

```bash
docker compose exec spark /opt/spark/bin/spark-submit /opt/spark/work-dir/spark/star_etl.py
```

3. Запустить Spark-джобу построения шести отчетов в ClickHouse:

```bash
docker compose exec spark /opt/spark/bin/spark-submit /opt/spark/work-dir/spark/clickhouse_reports.py
```

4. Проверить данные:

```bash
docker compose exec postgres psql -U admin -d bigdata -c "select count(*) from mock_data;"
docker compose exec postgres psql -U admin -d bigdata -c "select count(*) from fact_sales;"
docker compose exec clickhouse clickhouse-client --query "show tables"
```

Ожидаемые обязательные таблицы ClickHouse:
